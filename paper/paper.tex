...
form ($m_1=m_2$), with the distribution of component masses chosen to be $\propto m_1^{-p}$ 
 with $p=2$ between $3 M_\odot$ and $M_{max}(Z)$ and zero otherwise; see, e.g.,
...
different star-forming environments, not the overall normalization, so the overall scale of $\lambda$ is irrelevant.  
...
%$\lambda_o F_{max} < 3\times 10^{-4}/M_\odot$
 (see, e.g.\cite{popsyn-LIGO-SFR-2008}, \abbrvKBLowZa).  For the purposes of illustration, we adop a  concrete scale
factor  $
...
Even when electromagnetic counterparts are available, the long delay times between coalescence and merger  limit
