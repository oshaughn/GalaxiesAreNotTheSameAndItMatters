...
\citep{Munshi13}, halo properties \citep{Zolotov09,Zolotov10), and
  ...
  10^{10}$ \msun, and circular velocity $v_{circ} = xx$ km s$^{-1}$.
  ...
  \msun, and circular velocity $v_{circ} = xx$ km s$^{-1}$.
...
\caption{\label{fig:TwoGalaxies}\textbf{Star formation and metallicity versus time}:  \emph{Left panel}: Star formation history $\dot{M}_{*}$
  versus time.  Red corresponds to galaxy A (``boring'') and blue to gaalxy B (``exciting'').  
...
At merger, black holes may promptly engulf a companion neutron star \editremark{same papers as for Brandon:Foucart,
  etc}.  Without  a tidal disruption event prior to merger, the prospects for associated electromagnetic counterparts
...
** May want more Z bins to make sure we resolve it. Our differences between predictions are not huge

*** or rewrite script to use numpy on raw star data.  Use 'starmetal.dat': last colum is age.

** Clear that low-Z matters a lot
